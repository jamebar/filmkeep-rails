class Enrich
  ...
  def enrich
    results = Array.wrap(data).each do |d|
      assign_values(d) if d.has_key? "tmdb_id"

      assign_values(d['film']) if  d.has_key? "film"

      if d.has_key? "films"
        d['films'].each {|f| assign_values(f) if f.has_key? "tmdb_id" }
      end
    end
